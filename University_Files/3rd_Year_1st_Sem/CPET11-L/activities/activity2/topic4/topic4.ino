...
// Always reset back to 1200 when entering reset mode
countdown_time = 1200;
...
while (digitalRead(button1)==HIGH)  {
  ...
  // Decrement (one second, borrow to minutes when needed)
  if (digitalRead(button3)==LOW) { 
    if (pressed3 == 0 || pressed3 > 30) {
      ...
      if (minutes > 0 || seconds > 0) {
        ...
        countdown_time = minutes * 100 + seconds;
        dig = IntToDigits(countdown_time);
      }
    } 
    pressed3 += 1;
  }
  // Increment (one second, carry to minutes at 59)
    ...
    if (pressed4 == 0 || pressed4 > 30) {
      ...
      if (!(minutes == 99 && seconds == 59)) {
        if (seconds == 59) {
          if (minutes < 99) { minutes++; seconds = 0; }
        } else {
          seconds++;
        }
        countdown_time = minutes * 100 + seconds;
...
6 2 7 9 10 3
A0 5 - 4 8 11
220ohm for Display segment pins

Button 1= 13
Button 2= 12
Button 3= 16
Button 4= 17
no pull up, one on pin, one on gnd
